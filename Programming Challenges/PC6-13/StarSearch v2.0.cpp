...
void getJudgeData(double theArray[], int theSize) {
	...
	for (int i = 0; i < theSize; i++) {
		do {//Input validation loop.
			cout << "Please input a score between 0 and 10. Decimals are allowed: ";
...
double findHighest(const double theArray[], int theSize) {//Same general principle as the find lowest. 
...
double sumOfFive(const double theArray[], int theSize, double a, double b) {
	...
	for (int i = 0; i < theSize; i++) {//Add everything in the array. It's just an accumulator.
		result += theArray[i];
	}
